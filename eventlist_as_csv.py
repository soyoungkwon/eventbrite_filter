...
def main(url, max_pages):
    ...
    weblinks, eventlists = trade_spider(url, max_pages)

    # change into pandas
    event_data = {'Event': eventlists}
    event_pd = pd.DataFrame(event_data)#, weblinks_data)
    ...
    event_pd_clean.to_csv(path_curr +'eventlist_csv/' + 'eventlist_by_page/' + '1.csv', encoding = 'utf-8', index=False)
...
def extract_event(weblinks):
    ...
    for event in range(n_event):
        test = requests.get(weblinks[event])
        # page_content = test.content
        soup = BeautifulSoup(test.content, 'html.parser')
        string_content = soup.find('div', {'class': 'js-display-price'}).text
